...
export const BattleMovedex: {[moveid: string]: MoveData} = {
	...
	drainingfang: {
		...
		onPrepareHit: function (target, pokemon, move) {
			this.attrLastMove('[still]');
			this.add('-anim', pokemon, "Crunch", target);
		},
		drain: [1, 2],
		secondary: null,
		target: "normal",
	...
	meditate: {
		...
		shortDesc: "Raises the user's Attack, Defense and Special Defense by 1.",
		isNonstandard: "Past",
		name: "Meditate",
		pp: 30,
		priority: 0,
	...
	simplebeam: {
		...
		basePower: 0,
		category: "Status",
		desc: "Causes the user's Ability to become Simple. Fails if the user's Ability is Battle Bond, Comatose, Disguise, Multitype, Power Construct, RKS System, Schooling, Shields Down, Simple, Stance Change, Truant, or Zen Mode.",
		shortDesc: "The user's Ability becomes Simple.",
		name: "Simple Beam",
		pp: 15,
		priority: 0,
		flags: {protect: 1, reflectable: 1, mirror: 1, mystery: 1},
	...
	spinkick: {
		...
		onAfterHit(target, pokemon) {
			...
			for (const condition of sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
					this.add('-sideend', pokemon.side, this.dex.getEffect(condition).name, '[from] move: Spin Kick', '[of] ' + pokemon);
				}
			}
			if (pokemon.hp && pokemon.volatiles['partiallytrapped']) {
				pokemon.removeVolatile('partiallytrapped');
