...
export const BattleAbilities: {[abilityid: string]: AbilityData} = {
	...
	galvanize: {
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.galvanizeBoosted) return this.chainModify([0x14CD, 0x1000]);
		},
		name: "Galvanize",
		rating: 4,
		num: 206,
	...
	swarm: {
		desc: "This Pokemon's attacking stat is multiplied by 1.5 while using a Bug-type attack.",
	...
	swordsman: {
		desc: "This Pokemon's Slash-based attacks have their power multiplied by 1.5.",
		shortDesc: "This Pokemon's Slash-based attacks have 1.5x power.",
		onBasePowerPriority: 19,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags['slash']) {
				return this.chainModify(1.5);
	...
	vitalspirit: {
		...
		onModifyType(move, pokemon) {
			...
			if (move.type === 'Normal' && !noModifyType.includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Fighting';
				move.vitalspiritBoosted = true;
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.vitalspiritBoosted) return this.chainModify([0x14CD, 0x1000]);
		},
		name: "Vital Spirit",
		rating: 2,
		num: 72,
